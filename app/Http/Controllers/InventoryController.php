...
class InventoryController extends Controller
{
    ...
    public function invStatsSimpleDetailedGiac(Request $req, $id=null)
    {
        if (!empty($id)) {
            $req->session()->put('inventory.session.id', $id);
        } else {
        ...
        $invSession = ($req->session()->has('inventory')) ? InventorySession::find($req->session()->get('inventory.session.id')) : null;
        return view('mcslide.inventory.stats_simple_detailed-giac', ['invSession' => $invSession]);
    }
    ...
    public function exportXlsSimpleTotal(Request $req)
    {
        // dd();
        $inv_ids = $req->session()->get('invsimple.xlsExport.inv_ids');
        $req->session()->forget('invsimple.xlsExport.inv_ids');
        $filename = 'InvSimpleTotal_Export_' . Carbon::now()->format('YmdHis') . '.xlsx';
        return Excel::download(new InventorySimpleTotalExport($inv_ids), $filename);
    }
