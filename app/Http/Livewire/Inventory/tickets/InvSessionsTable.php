    ...
    public function columns(): array
    {
        ...
        if (Laratrust::isAbleTo('config-update')) {
            array_push(
                ...
                ->title(fn ($row) => '<span class="fa fa-print pr-1"></span>Genera Tagliandini/Magazzino')
                ->location(fn ($row) => '#')
                    ->attributes(function ($row) {
                        return [
                            'class' => 'btn btn-outline-success btn-xs mr-2 ',
                            'onclick' => "Livewire.emit('modal.open', 'inventory.tickets.inv-tickets-modal-print', {'invSession_id': " . $row->id . "});"
                        ];
                    }),
            );
        }
